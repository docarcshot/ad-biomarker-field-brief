...
import { landscapeView, meetingView } from './views.mjs';
...
const js = fs.readFileSync(path.join(root, 'src/view-state.js'), 'utf8') + '\n' + fs.readFileSync(path.join(root, 'src/app.js'), 'utf8');
...
const overdue = ADBFViewState.delayed(status, new Date().toISOString().slice(0,10));
const clientState = JSON.stringify({ids:entries.map(entry=>entry.id),status:{reviewedThrough:status.reviewedThrough,nextScheduledReview:status.nextScheduledReview,workflowState:status.workflowState}}).replace(/</g,'\\u003c');
...
const quickReferences = [
  ['AD clinical guidance','Diagnosis, staging, treatment, and care guidance','https://www.alz.org/professionals/health-systems-medical-professionals/clinical-practice-guidelines-and-evidence'],
  ['FDA drug labels','Authoritative treatment indications, dosing, and safety','https://www.accessdata.fda.gov/scripts/cder/daf/'],
  ['WHO dementia risk reduction','2026 guideline and recommendations','https://www.who.int/publications/i/item/9789240123557'],
  ['FDA device databases','Clearances, De Novo classifications, and device records','https://www.fda.gov/medical-devices/device-advice-comprehensive-regulatory-assistance/medical-device-databases'],
  ['Medicare Coverage Database','National and local coverage documents and coding articles','https://www.cms.gov/medicare-coverage-database/'],
  ['CMS laboratory fee schedule','Medicare payment files and HCPCS laboratory codes','https://www.cms.gov/medicare/payment/fee-schedules/clinical-laboratory-fee-schedule-clfs'],
  ['ClinicalTrials.gov','Registered Alzheimer biomarker and treatment studies','https://clinicaltrials.gov/'],
...
const nav = current => [
  ['Latest','/'],['Archive','/archive/'],['Landscape','/landscape/'],['Meeting prep','/meeting/'],['Methods','/methods/'],['About','/about/'],['RSS','/feed.xml']
...
<body><a class="skip-link" href="#content">Skip to content</a><header class="site-header"><div class="header-inner"><a class="brand" href="/"><span class="brand-mark" aria-hidden="true">Aβ</span><span>${site.name}</span></a><nav class="site-nav" aria-label="Primary">${nav(current)}</nav><button class="theme-toggle" type="button" data-theme-toggle aria-label="Change color theme">◐</button></div></header><main id="content"><aside class="review-alert" data-review-warning role="status"${overdue?'':' hidden'}>Review delayed. Evidence was last reviewed through ${fmt(status.reviewedThrough)}. <a href="/methods/#review-log">Review log</a></aside>${body}</main><footer class="site-footer"><div class="footer-inner"><p>${disclosure}</p><div class="footer-links"><a href="/methods/">Methods</a><a href="/feed.xml">RSS</a></div></div></footer><script type="application/json" id="adbf-view-state">${clientState}</script><script src="${jsPath}" defer></script></body></html>`;

const card = (entry, {article=false, extra=false}={}) => {
  const resultLabel = entry.resultType === 'recommendations' ? 'Key recommendations' : entry.resultType === 'regulatory' ? 'Key label details' : 'Key quantitative results';
  const attrs = {
    topic:entry.topics, modality:entry.modalities, biomarker:entry.biomarkers, clinicalUse:entry.clinicalUses,
  ...
  const search = [entry.title,entry.bottomLine,entry.brief30,entry.studyDesign,...entry.keyResults,...entry.topics,...entry.modalities,...entry.biomarkers,...entry.clinicalUses,...entry.assays,...entry.platforms,...entry.organizations,entry.regulatoryStatus,entry.evidenceSource].join(' ').toLowerCase();
  const tags = [
    ...entry.topics.map(value=>({name:'topic',value})),
  ...
  return `<article class="brief-card${article?' entry-full':''}" data-entry-id="${escapeAttr(entry.id)}" data-relevance="${escapeAttr(entry.relevance)}" ${extra?'data-recent-extra hidden':''} ${dataAttrs} data-search="${escapeAttr(search)}" data-source-date="${entry.sourceDate}" data-date-added="${entry.dateAdded}"${article?'':' data-archive-card'}>
    ...
    <p class="number-line"><span class="visually-hidden">${resultLabel}: </span>${escapeHTML(entry.keyResults[0])}</p>
    <div class="article-tags" aria-label="Article tags">${tagHTML}</div><p class="added">Added ${fmt(entry.dateAdded)} · ${escapeHTML(entry.identifier)}</p><button class="meeting-toggle" type="button" data-meeting-toggle="${escapeAttr(entry.id)}" data-brief-title="${escapeAttr(entry.title)}" aria-pressed="false" aria-label="Add ${escapeAttr(entry.title)} to meeting">Add to meeting</button></div>
    <div class="card-details" aria-label="Brief sections">
      <details class="brief-section" data-entry-id="${escapeAttr(entry.id)}" data-brief-section="summary"><summary><span>Evidence summary</span><small>Design, results, and what changed</small></summary><div class="section-body">
        <h4>Thirty-second field brief</h4><p>${escapeHTML(entry.brief30)}</p><h4>${entry.resultType === 'recommendations' ? 'Evidence basis and population' : 'Study design'}</h4><p>${escapeHTML(entry.studyDesign)}</p>
        <h4>${resultLabel}</h4><ul class="results-list">${entry.keyResults.map(x=>`<li>${escapeHTML(x)}</li>`).join('')}</ul>
      ...
      <details class="brief-section" data-entry-id="${escapeAttr(entry.id)}" data-brief-section="impact"><summary><span>Clinical and field impact</span><small>Interpretation, implementation, and access</small></summary><div class="section-body">
        ...
        ${entry.platformRelevance?`<h4>Quanterix or platform relevance</h4><p>${escapeHTML(entry.platformRelevance)}</p>`:''}<h4>Access or reimbursement relevance</h4><p>${escapeHTML(entry.accessRelevance)}</p>
      </div></details>
      ...
      <div class="source-block"><p><a href="${escapeAttr(entry.primarySource)}" rel="noopener">Primary source</a>${entry.doi?` · <a href="https://doi.org/${escapeAttr(entry.doi)}" rel="noopener">DOI</a>`:''}${(entry.supportingSources||[]).map(source=>` · <a href="${escapeAttr(source.url)}" rel="noopener">${escapeHTML(source.label)}</a>`).join('')}</p><p class="citation">${escapeHTML(entry.citation)}</p>
...
const historical = entries.filter(e=>e.historical).sort((a,b)=>b.dateAdded.localeCompare(a.dateAdded)||b.sourceDate.localeCompare(a.sourceDate)).slice(0,3);
...
const home = `${coverage.scopeExpansion?`<p class="scope-update">Now covering clinical trials, general AD guidelines, and management. <a href="/archive/?topic=clinical+trials">View clinical trials</a> · <a href="/archive/?topic=guidelines">View guidelines</a> · <a href="/archive/?topic=management">View management</a> · <a href="/coverage/">Backfill details</a></p>`:''}<section class="masthead"><div><p class="eyebrow">Independent evidence monitoring</p><h1>${site.name}</h1><p class="lede">${site.description}</p><form class="home-search" action="/archive/" role="search"><label class="visually-hidden" for="home-q">Search the archive</label><input id="home-q" name="q" type="search" placeholder="Search biomarkers, treatments, guidelines, or results"><button class="primary-button" type="submit">Search</button></form></div>
<aside class="status-panel" aria-label="Review status"><dl><div><dt>Reviewed through</dt><dd>${fmt(status.reviewedThrough)}</dd></div><div><dt>Qualified</dt><dd>${status.newItemsQualified}</dd></div><div><dt>Next review</dt><dd>${fmt(status.nextScheduledReview)}</dd></div></dl><p data-review-message data-normal-message="${escapeAttr(statusText)}" class="status-message${overdue?' delayed':''}">${overdue?'Review delayed':escapeHTML(statusText)}</p></aside></section>
...
<div class="latest-layout"><div class="latest-feed">${relevanceLegend()}${prospective.length?`<div class="card-list">${prospective.slice(0,10).map((e,i)=>card(e,{extra:i>=3})).join('')}</div>${prospective.length>3?`<div class="show-controls"><button data-show-more>Show more recent</button><button data-show-fewer hidden>Show fewer</button></div>`:''}`:`<div class="callout"><strong>No prospective entries yet.</strong> The latest routine review added no post-launch items. Verified backfill appears below and in the archive.</div>`}</div>${quickReferencePanel()}</div></section>
...
<section class="archive-tools" aria-label="Archive controls"><div class="topic-tools">${select('topic','Topic',['Biomarkers','Clinical Trials','Registry Results','Guidelines','Management'])}<p>Choose a topic to separate biomarker-focused research, published interventional trial reports, exceptional registry-only results, guidance, and general AD care. Briefs may have more than one topic.</p></div><div class="tool-row"><label class="field"><span>Full-text search</span><input class="search-input" type="search" name="q" data-filter placeholder="Title, result, treatment, assay, organization…"></label><label class="field"><span>Sort by</span><select name="sort" data-filter><option value="added-desc">Most recently added</option><option value="source-desc">Newest source date</option><option value="source-asc">Oldest source date</option></select></label><button type="button" data-open-filters class="filter-toggle">Filters</button></div>
<div class="filter-drawer" data-filter-drawer><div class="filter-drawer-head"><strong>Filters</strong><div class="filter-drawer-actions"><button type="button" class="clear-filters" data-clear-all>Clear all</button><button type="button" class="filter-done" data-close-filters>Done</button></div></div><div class="filter-grid">${select('modality','Modality',unique('modalities'))}${select('biomarker','Biomarker',unique('biomarkers'))}${select('clinicalUse','Clinical use',unique('clinicalUses'))}${select('relevance','Relevance label',unique('relevance'))}</div></div><div class="active-filters" data-active-filters aria-live="polite"></div></section>
...
mkdirWrite('landscape/index.html',layout({title:'Landscape',current:'Landscape',canonical:'/landscape/',body:landscapeView(landscape,entries)}));
mkdirWrite('meeting/index.html',layout({title:'Meeting preparation',current:'Meeting prep',canonical:'/meeting/',body:meetingView(entries),extraHead:'<meta name="robots" content="noindex,follow">'}));

const registryHTML = sources.map(source=>`<article class="registry-item"><h3><a href="${escapeAttr(source.url)}">${escapeHTML(source.name)}</a></h3><p><strong>${escapeHTML(source.category)}</strong> · ${escapeHTML(source.cadence)} · Last checked ${fmt(source.lastChecked)}${source.lastCheckedScope?` (${escapeHTML(source.lastCheckedScope)})`:''}${source.targetedManagementCheck?` · Targeted management check ${fmt(source.targetedManagementCheck)}`:''}</p><p>${escapeHTML(source.method)}</p></article>`).join('');
const reviewLogHTML = [...status.reviewLog].reverse().map(run=>`<article class="registry-item"><h3>${fmt(run.reviewedThrough)}</h3><p><strong>${run.outcome==='success'?'Successful review':'Review delayed'}</strong> · ${run.qualified} qualified</p><p>${escapeHTML(run.note)}</p></article>`).join('');
...
<h2>Site specification</h2><dl class="definition-grid"><div><dt>Final name</dt><dd>AD Biomarker Field Brief</dd></div><div><dt>One-sentence description</dt><dd>${escapeHTML(site.description)}</dd></div><div><dt>Information architecture</dt><dd>Latest, Archive, Landscape, Meeting prep, Methods, About, and RSS; historical coverage is linked from Methods and Archive.</dd></div><div><dt>Visual system</dt><dd>Scientific-publication typography, cool neutral surfaces, dark teal accent, text-labeled evidence states, 1100-pixel maximum width, and system light/dark modes.</dd></div></dl>
<h2>Inclusion threshold</h2><p>Zero to three entries may be added per review. A development qualifies only if it could plausibly change scientific exchange, AD diagnosis or staging, biomarker interpretation or test selection, treatment choice, eligibility, dosing, safety monitoring, symptom management, nonpharmacologic care, caregiver support, implementation, authorization, coverage, access, or trial design. General AD guidelines and management evidence can qualify independently of a biomarker consequence. Routine association studies, unconfirmed claims, and research without a material clinical or field implication are excluded.</p>
<h2>Evidence model</h2><p>Primary sources control. Analytical validity, clinical validity, clinical utility, regulatory authorization, availability, guideline endorsement, and payer coverage are separate fields. Diagnostic results include the actual denominator, reference standard, thresholds, indeterminate range, predictive values, and confidence intervals when the source reports them. Treatment studies report population, design, absolute benefit and harm, follow-up, denominator, and uncertainty when available, and distinguish primary from secondary or exploratory findings. Guidelines identify issuing body, population, recommendation, evidence basis, strength when available, and formal guideline versus consensus status. Guideline recommendations and regulatory details are not invented study results. Cross-study performance is never presented as a head-to-head comparison.</p>
<h2>Topic classification</h2><p>Biomarkers is limited to reports whose central question or primary analysis concerns biomarker measurement, validity, interpretation, implementation, authorization, or coverage. A treatment trial is not classified as biomarker research merely because it measures biomarkers. Clinical Trials identifies primary reports of prospective interventional human trials; Registry Results identifies the rare useful result available only in a public trial registry. It is not an enrollment feed. Evidence syntheses, observational cohorts, regulatory summaries, and guidelines are not trial reports. Topics overlap only when each independently reflects a central purpose of the report.</p>
<h2>Structured entry schema</h2><p>Every entry is stored as validated JSON and includes stable ID/slug, source and added dates, evidence and relevance labels, topic/modality/biomarker/use tags, design or evidence basis, quantitative results or recommendations, interpretation, questions and answers, limitations, platform relevance when applicable, access relevance, source, citation, and correction history. General management entries may leave inapplicable assay fields empty. The same data generate cards, permalinks, archive filters, sitemap, and RSS.</p>
<h2>Historical backfill</h2><p>The original biomarker backfill covered January 1 through September 4, 2026 with a completed month-by-month source-category audit. The September 7 scope expansion adds a separately recorded, targeted 2026 backfill for guidelines and management; it does not extend that original completion claim to all AD care. Historical additions are labeled by their actual source dates, deduplicated against existing entries, and excluded from prospective per-review counts. Zero-result months remain visible. <a href="/coverage/">Inspect the coverage audit.</a></p>
...
<h2>Validation and deployment</h2><p>Before deployment, the build checks required fields, ISO dates, relevance tags, duplicate stable identifiers and slugs, HTTPS primary sources, nonempty findings appropriate to the evidence type, landscape verification dates, and coverage-audit consistency. CSS, JavaScript, entry data, and status data receive content hashes. The deployment package, RSS, sitemap, archive, cards, and landscape are generated in one build.</p>
...
<h2 id="review-log">Review log</h2></div><div class="registry">${reviewLogHTML}</div><div class="prose">
...
const rssItems = [...entries].sort((a,b)=>b.dateAdded.localeCompare(a.dateAdded)||b.sourceDate.localeCompare(a.sourceDate)).slice(0,30).map(entry=>`<item><title>${escapeHTML(entry.title)}</title><link>${abs(`/entries/${entry.slug}/`)}</link><guid isPermaLink="true">${abs(`/entries/${entry.slug}/`)}</guid><pubDate>${new Date(`${entry.dateAdded}T12:00:00Z`).toUTCString()}</pubDate><description>${escapeHTML(entry.bottomLine)}</description>${entry.topics.map(topic=>`<category>${escapeHTML(topic)}</category>`).join('')}</item>`).join('');
...
const sitemapUrls = ['/', '/archive/','/landscape/','/meeting/','/methods/','/coverage/','/about/',...entries.map(e=>`/entries/${e.slug}/`)];
...
console.log(`Built ${entries.length} entry pages and 7 core pages with hashed assets.`);
