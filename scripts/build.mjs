...
${prospective.length?`<div class="card-list">${prospective.slice(0,10).map((e,i)=>card(e,{extra:i>=3})).join('')}</div>${prospective.length>3?`<div class="show-controls"><button data-show-more>Show more recent</button><button data-show-fewer hidden>Show fewer</button></div>`:''}`:`<div class="callout"><strong>No prospective entries yet.</strong> The launch-date review completed successfully and no post-launch item met the inclusion threshold.</div>`}</section>
